...
Astro + React islands + Tailwind CSS v4, Content Collections como fuente de
datos inicial detrás de un adapter intercambiable.
...
- **Persistencia real** — Content Collections es solo lectura en build time. El panel de albergues y el registro de donaciones necesitan un backend/DB antes de ir a producción; el `DataAdapter` (`src/lib/data/adapter.ts`) ya aísla ese cambio del resto de la UI.
- **Autenticación** — `/panel/*` no tiene sesión real todavía.
